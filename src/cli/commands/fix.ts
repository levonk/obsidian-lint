    ...
    .action(async (vaultPath: string, options: any, command: Command) => {
      try {
        ...
        if (globalOptions.verbose) {
          console.log(chalk.blue('Running fix command with options:'));
          console.log(
            chalk.gray(
              JSON.stringify(
        ...
        const processOptions: ProcessOptions = {
          ...
          fix: true, // fix command enables fixing
        ...
        if (!globalOptions.json && result.fixesApplied.length > 0) {
          console.log(
            chalk.green(`\n✓ Applied ${result.fixesApplied.length} fixes`)
          );
        }
      } catch (error) {
        console.error(
          chalk.red('Fix failed:'),
